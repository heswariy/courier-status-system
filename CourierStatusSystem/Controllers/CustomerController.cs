...
namespace CourierStatusSystem.Controllers
{
    public class CustomerController : Controller
    {
        public async Task<IActionResult> Index()
        {
            ...
            {
                using (var response = await httpClient.GetAsync("https://localhost:44317/api/Customer"))
                {
        ...
        public async Task<IActionResult> GetCustomer(int id)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.GetAsync("https://localhost:44317/api/Customer/" + id))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> AddCustomer(Customer customer)
        {
            if (ModelState.IsValid)
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    using (var response = await httpClient.PostAsync("https://localhost:44317/api/Customer", content))
                    {
                        string apiResponse = await response.Content.ReadAsStringAsync();
                        customer = JsonConvert.DeserializeObject<Customer>(apiResponse);
                    }
        ...
        public async Task<IActionResult> UpdateCustomer(Customer customer)
        {
            ...
            if (ModelState.IsValid)
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    using (var response = await httpClient.PutAsync("https://localhost:44317/api/Customer", content))
                    {
                        string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> DeleteCustomer(int CustomerId)
        {
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.DeleteAsync("https://localhost:44317/api/Customer/" + CustomerId))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
                }
            }
